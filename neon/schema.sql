-- FamilyBudget / Lena Money schema for Neon
-- Apply in the Neon SQL Editor on project FamilyBudget (direct connection).
-- Not applied yet. Super-admin and Neon Auth stay unused by the app.
--
-- Difference vs Supabase:
--   profiles.id is a uuid primary key without FK to auth.users.
--   Neon Auth is not wired. auth.uid() is created only if missing, so the
--   current_profile view and RLS policies can exist.
--   No GRANT to roles anon/authenticated: those roles do not exist on Neon
--   unless Data API created them. The Vercel function uses DATABASE_URL
...
-- 2) User profiles (uuid matches auth user id when Auth is added later)
...
-- Snapshot table: no public anon policy. Access only via /api/snapshot + DATABASE_URL.
